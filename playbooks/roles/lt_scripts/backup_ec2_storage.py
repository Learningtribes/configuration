...
def backup_volume_job(region, region_tag):
    snapshot_list = []
    ...
    if response_volumes['Volumes']:
        for volume in response_volumes['Volumes']:
            if volume['State'] == 'in-use':
                ...
                response_createsnapshot = region.create_snapshot(Description='snapshot from '+volume['VolumeId']+'('+volume['Attachments'][0]['InstanceId']+') at '+create_snapshot_date, VolumeId=volume['VolumeId'], TagSpecifications=[{'ResourceType':'snapshot', 'Tags':[{'Key': 'backup-type', 'Value': 'hawthorn'}, {'Key': 'Region', 'Value': region_tag}]}])
...
def copy_snapshot_job(region, dest_region, source_region_string, region_tag):
    snapshot_list = []
    copy_snapshot_list = []
    response_snapshots = region.describe_snapshots(Filters=[{'Name': 'tag:backup-type', 'Values': ['hawthorn']}])
    if response_snapshots['Snapshots']:
        for snapshot in response_snapshots['Snapshots']:
            if snapshot['StartTime'] > pytz.UTC.localize(datetime.datetime.combine(datetime.date.today(), datetime.time.min)):
                ...
                response_copysnapshot = dest_region.copy_snapshot(Description=snapshot['Description'], SourceRegion=source_region_string, SourceSnapshotId=snapshot['SnapshotId'], Encrypted=True, TagSpecifications=[{'ResourceType':'snapshot', 'Tags':[{'Key': 'Region', 'Value': region_tag}]}])
...
backup_volume_job(ec2_eu_ie, 'EU')
delete_local_old_snapshot_job(ec2_eu_ie, 15)
copy_snapshot_job(ec2_eu_ie, ec2_eu_se, ec2_eu_ie_string, 'EU')
delete_remote_old_snapshot_job(ec2_eu_se, 15)

backup_volume_job(ec2_us_east, 'US')
delete_local_old_snapshot_job(ec2_us_east, 15)
copy_snapshot_job(ec2_us_east, ec2_us_west, ec2_us_east_string, 'US')
